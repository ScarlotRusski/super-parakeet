...
cls
color 07
title  Microsoft Activation Menu
mode 76, 30
...
for /f "skip=2 tokens=2*" %%a in ('"reg query %_PRIDs% /v @setlocal DisableDelayedExpansion
@echo off
...
::End::Configuration" %_Nul6%') do set "_PRIDs=%_PRIDs%\%%b"
if "%_ProductIds%"=="" (
if %_Office15% EQU 0 (echo Error: Office C2R ProductIDs are not detected&goto :%_fC2R%) else (goto :Reg15istry)
)
if not exist "%_LicensesPath%\ProPlus*.xrm-ms" (
